...
async function main() {
  ...
  const proxyAddress = "0x05128Fcfc219Ca32A8eb6C9e4423AfDdB5A41D76";

  console.log(`Lock2 deployed to ${newImplementationContract.address}\n`);

  const ProxyLock = await hre.ethers.getContractFactory("ProxyLock");

  const Proxy = ProxyLock.attach(proxyAddress);
  const upgradeTX = await Proxy.connect(adminWallet).upgrade(
